...
def get_phq9_target(row: dict[str, str]) -> float:
    # 先尝试找 Track 1 (Elder) 的 "PHQ-9"
    if "PHQ-9" in row and str(row["PHQ-9"]).strip() != "":
        return float(row["PHQ-9"])
    
    # 如果没找到，再尝试找 Track 2 (Young) 的 "phq9_score"
    if "phq9_score" in row and str(row["phq9_score"]).strip() != "":
        return float(row["phq9_score"])
        
    # 如果都没找到，才抛出报错（或者返回 0.0 防止崩溃）
    raise KeyError("Column 'PHQ-9' or 'phq9_score' not found in split csv")
...
def _feature_aliases(modality: str, feature_name: str) -> tuple[str, ...]:
    if modality == "audio":
        return AUDIO_FEATURE_ALIASES.get(feature_name, (feature_name,))
    if modality == "video":
        return VIDEO_FEATURE_ALIASES.get(feature_name, (feature_name,))
    raise ValueError(f"Unknown modality={modality}")


def _resolve_peer_feature_file(
    base_file: Path,
    base_root: Path,
    modality: str,
    feature_name: str,
) -> Path:
    rel_path = base_file.relative_to(base_root)
    modality_root = base_root.parent
    aliases = _feature_aliases(modality, feature_name)
    candidates: list[Path] = []

    for alias in aliases:
        candidate = modality_root / alias / rel_path
        candidates.append(candidate)
        if feature_name == "openface" and rel_path.stem.startswith("event_"):
            candidates.append(modality_root / alias / rel_path.parent / rel_path.stem / f"{rel_path.stem}_all.npy")
...
class MPDDElderDataset(Dataset):
    def __init__(
        ...
        if self.need_av:
            ...
            # --- 新增：代理侦察兵逻辑 ---
            # 如果是全模态，就用 mfcc 和 densenet 的文件夹去“点名”找文件
            ...
            for split_name in ("train", "test"):
                ...
                # 用 search_audio 和 search_video 替换原来的 audio_feature 和 video_feature
                audio_root, audio_alias = _resolve_feature_dir(split_root, "audio", split_name, search_audio)
                video_root, video_alias = _resolve_feature_dir(split_root, "video", split_name, search_video)
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
            ...
            if self.need_av:
                mfcc_pairs, opensmile_pairs, wav2vec_pairs = [], [], []
                densenet_pairs, resnet_pairs, openface_pairs = [], [], []
                ...
                # 获取当前 split 的根目录，用于后续安全地重构路径
                audio_root = self.audio_roots[source_split]
                video_root = self.video_roots[source_split]
                
                # 动态判断 wav2vec 的真实维度 (Track1:768, Track2:1024)
                w2v_dim = 1024 if self.is_young else 768

                for pair_idx in pair_indices:
                    a_file = audio_map[pair_idx]
                    v_file = video_map[pair_idx]

                    # 读取并 Resize 音频特征 (Fallback 维度与官方严格对齐)
                    mfcc_pairs.append(_resize(_load_feature_array(_resolve_peer_feature_file(a_file, audio_root, "audio", "mfcc"), fallback_dim=64), self.target_t))
                    opensmile_pairs.append(_resize(_load_feature_array(_resolve_peer_feature_file(a_file, audio_root, "audio", "opensmile"), fallback_dim=65), self.target_t))
                    wav2vec_pairs.append(_resize(_load_feature_array(_resolve_peer_feature_file(a_file, audio_root, "audio", "wav2vec"), fallback_dim=w2v_dim), self.target_t))

                    # 读取并 Resize 视频特征 (Fallback 维度与官方严格对齐)
                    densenet_pairs.append(_resize(_load_feature_array(_resolve_peer_feature_file(v_file, video_root, "video", "densenet"), fallback_dim=1000), self.target_t))
                    resnet_pairs.append(_resize(_load_feature_array(_resolve_peer_feature_file(v_file, video_root, "video", "resnet"), fallback_dim=1000), self.target_t))
                    openface_pairs.append(_resize(_load_feature_array(_resolve_peer_feature_file(v_file, video_root, "video", "openface"), fallback_dim=710), self.target_t))
                    
                    pair_mask.append(1.0)

                # 将片段数量填充到 PAIR_COUNT (4个)，缺失的部分用全 0 补齐
                while len(pair_mask) < PAIR_COUNT:
                    mfcc_pairs.append(torch.zeros(self.target_t, 64, dtype=torch.float32))
                    opensmile_pairs.append(torch.zeros(self.target_t, 65, dtype=torch.float32))
                    wav2vec_pairs.append(torch.zeros(self.target_t, w2v_dim, dtype=torch.float32))
                    densenet_pairs.append(torch.zeros(self.target_t, 1000, dtype=torch.float32))
                    resnet_pairs.append(torch.zeros(self.target_t, 1000, dtype=torch.float32))
                    openface_pairs.append(torch.zeros(self.target_t, 710, dtype=torch.float32))
                    pair_mask.append(0.0)

                # 存入 result 字典，传给模型的前向传播
                result["mfcc"] = torch.stack(mfcc_pairs)
                result["opensmile"] = torch.stack(opensmile_pairs)
                result["wav2vec"] = torch.stack(wav2vec_pairs)
                result["densenet"] = torch.stack(densenet_pairs)
                result["resnet"] = torch.stack(resnet_pairs)
                result["openface"] = torch.stack(openface_pairs)
