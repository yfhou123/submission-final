...
from dataset import GAIT_KEEP_DIM, PAIR_COUNT, _load_feature_array, _resize, resolve_project_path
...
DEFAULT_AV_CHUNK_CACHE_DIR = "Elder-trainval/cached_av_chunks_c4_t128_mfcc2600"
AV_KEYS = ("mfcc", "opensmile", "wav2vec", "densenet", "resnet", "openface")


def _default_cache_dir(data_root: str | Path) -> Path:
    return resolve_project_path(data_root) / "cached_av_chunks_c4_t128_mfcc2600"


class MPDDElderTextChunkCachedDataset(MPDDElderTextDataset):
    def __init__(
        self,
        *args: Any,
        ...
        **kwargs: Any,
        ...
        if av_cache_dir is None:
            av_cache_dir = _default_cache_dir(self.data_root)
    ...
    def _load_cached_av(self, person_id: int) -> dict[str, torch.Tensor]:
        ...
        if not cache_path.is_file():
            raise FileNotFoundError(
                f"Chunked A/V cache not found for id={person_id}: {cache_path}. "
                "Run feature_extract/cache_elder_av_chunks.py before Elder chunk-attention training."
            )
        try:
        ...
        for key in AV_KEYS:
            if key not in payload:
                raise KeyError(f"Missing key '{key}' in chunked A/V cache: {cache_path}")
            result[key] = payload[key].float()
        for key in ("pair_mask", "chunk_mask", "chunk_count"):
            if key not in payload:
                raise KeyError(f"Missing key '{key}' in chunked A/V cache: {cache_path}")
        result["pair_mask"] = payload["pair_mask"].float()
        result["chunk_mask"] = payload["chunk_mask"].float()
        result["chunk_count"] = payload["chunk_count"].long()
        return result
...
def infer_input_dims(dataset: MPDDElderTextChunkCachedDataset) -> dict[str, int]:
    sample = dataset[0]
    return {
        ...
        "text_dim": int(sample["text_segments"].shape[-1]) if "text_segments" in sample else TEXT_EMBED_DIM,
        "text_max_segments": int(sample["text_segments"].shape[-2]) if "text_segments" in sample else TEXT_MAX_SEGMENTS,
    }
