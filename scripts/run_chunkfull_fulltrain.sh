...
DEVICE="${DEVICE:-cuda}"
TASK="${TASK:-binary}"
TRAIN_SEEDS="${TRAIN_SEEDS:-1009 777 521 2701 3583 4441}"
...
CHECKPOINTS_DIR="${CHECKPOINTS_DIR:-checkpoints/fulltrain_chunkfull}"
LOGS_DIR="${LOGS_DIR:-logs/fulltrain_chunkfull}"
EXP_NAME="${EXP_NAME:-elder_chunkfull_c4t128_fulltrain}"
...
if [[ "$TASK" != "binary" && "$TASK" != "ternary" ]]; then
  echo "TASK must be binary or ternary, got: $TASK" >&2
  exit 1
fi
...
echo "=== Elder chunk-full full-train run ==="
echo "TASK=${TASK}"
echo "TRAIN_SEEDS=${TRAIN_SEEDS}"
echo "EPOCHS=${EPOCHS}"
echo "CHECKPOINTS_DIR=${CHECKPOINTS_DIR}"
echo "LOGS_DIR=${LOGS_DIR}"
echo "AV_CHUNK_CACHE_DIR=${AV_CHUNK_CACHE_DIR}"

for run_seed in $TRAIN_SEEDS; do
  run_exp_name="${EXP_NAME}_${TASK}_seed${run_seed}_ep${EPOCHS}"
  echo "[chunkfull-fulltrain][$TASK] seed=$run_seed epochs=$EPOCHS"
  "$PYTHON_BIN" train_text_coral_attn_chunk_fulltrain.py \
    --config config.json \
    --track Track1 \
    --task "$TASK" \
    --subtrack A-V+P \
